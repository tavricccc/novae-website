...
| Cloudflare Worker | 固定公開入口、CORS、Firebase token、webhook 簽章、Upstash fail-closed 限流 |
...
- 公開 action、登入同步與 Cloudinary webhook 會先在 Cloudflare 依來源／使用者執行限流；超額請求不進入 Supabase Edge Function。

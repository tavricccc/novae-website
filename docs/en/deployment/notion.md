...
This entire service is optional. Neon PostgreSQL remains the source of truth, and omitting Notion does not disable any core Novae workflow.

## Skip Notion

Do not create `NOTION_TOKEN`, `NOTION_DATABASE_ID`, or `NOTION_DATA_SOURCE_ID` in GitHub. The workflow sets `NOTION_ENABLED=false`, and the Worker safely skips every Notion operation.
...
Neon PostgreSQL remains the source of truth. Notion is an operations copy, not an authorization database or backup replacement.

...
3. Standard Cloudinary HMAC callback verification directly uses `CLOUDINARY_API_SECRET`; do not create a separate `CLOUDINARY_WEBHOOK_SECRET`.
...
All three values must belong to the same Product Environment, and the backend workflow healthcheck must complete successfully so the controlled preset is verified. Next: decide whether to [create the optional Notion copy](notion.md).

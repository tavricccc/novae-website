...
`config/rate-limits.config.json` 的現值包含：每人每天 10 件提案、10 件設備回報、20 則管理員公告與 50 張圖片，每小時 60 則留言，以及附議、公告按讚與「我也遇到」各 120 次。提案與公告各 5 張圖片、留言 1 張；圖片來源上限 20 MB，目標壓縮到 800 KB、最長邊 2000 px、WebP 初始品質 0.82。

Supabase 的 Upstash 規則另外限制登入同步、管理、刪除、偏好、Push token、健康檢查與背景 worker。Cloudflare 10／60 秒原生 burst limits 位於 `cloudflare/wrangler.toml`，不再由這份 JSON 或 Upstash 執行。調低前先確認真實流量；尤其不要用過低 IP 門檻誤傷共用校園網路。

標題 30 個字、提案／公告內文 1000 個可見字元、留言 70 個可見字元是固定的產品與資料庫規則，不在 `rate-limits.config.json` 內。圖片使用的 Markdown 標記只負責保存附件關聯，不計入上述可見文字長度；留言本身不解析其他 Markdown 格式。

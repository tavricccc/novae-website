...
repository 內的 `vercel.json` 已關閉 Vercel Git-based automatic deployment。正式發布由 GitHub Actions 控制，避免 Vercel 與 Actions 對同一個 commit 重複部署。
...
- [ ] project 已建立，且 Git-based automatic deployment 由 `vercel.json` 關閉。
- [ ] 已安全保存 `VERCEL_TOKEN`。

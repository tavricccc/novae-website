...
確認初始讀取成功、連線未被 CSP／代理阻擋、event audience 與目前使用者相符。提案、公告列表與留言會自動重連、連線失敗後補抓，並在前景定期校正；也可再次點擊目前所在的桌機頂部或手機底部導覽立即強制讀取列表。若強制讀取能恢復但自動更新仍持續失敗，應檢查 `realtime_events` publication、RLS 與瀏覽器 WebSocket 連線。

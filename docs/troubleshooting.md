...
首次設定先確認瀏覽器／作業系統的第一首選語言。若按下完成時短暫報錯、重試後顯示初始設定已完成，代表第一次請求可能已成功寫入但回應中斷；目前版本會刷新平台狀態並直接進入主畫面。若仍停在 Setup，檢查至少有一個提案與設備分類、最新 migration 已套用，並記錄 request ID。
...
6. 若 popup 被瀏覽器阻擋，系統會改用 redirect；回到站內時登入按鈕應維持「登入中…」直到同步完成。若仍可重複點擊、或顯示「登入回復逾時」「登入元件初始化失敗」，先重新整理一次，再檢查 Firebase authorized domain 與 Web App config。
7. 登入後仍停在登入卡、或點「提案」卻進「我的提案」：確認角色 bootstrap 與分類目錄是否成功；正常情況會等 bootstrap 完成後自動導向預設分類。

...
提交原始 JSON 與所有產生檔。分類與 API error code 必須同時符合前端、Cloudflare 與 Edge；精確業務限流由 JSON 產生給 Supabase，Cloudflare 原生 burst limit 則由 `wrangler.toml` 維護。不要手改 `generated/`。API 錯誤回應只能使用契約中的 code、request ID 與必要的 retry metadata，不能加入中文、英文句子或 provider exception。

...
- `docs/` / `docs/en/`：繁中與英文 Markdown 文件來源；`configuration.md`、`admin-guide.md` 與 `user-guide.md` 維護語言優先 Setup、動態提案／設備分類、分類優先授權與通知收件矩陣；`costs.md` 以同一組 2026-07-23 官方額度與 MAU／學年模型逐平台估算免費容量，並記錄 180 天結案硬刪除、Notion 長期副本、圖片快取／可變列表 no-store 與不自動停用功能的成本防線；`ui-design-system.md` 說明 Next／React 共用 primitive、三階陰影、響應式與 motion 契約；`contributing.md` 集中 `test:env`、`verify:stress` 與一般驗證規則；`architecture.md` 說明 App Router 前端邊界、統一 Media Gateway、Detail intent 預抓與一次性摘要 seed；`deployment/cloudflare.md` 說明 Worker、workers.dev、API token、媒體入口與 CORS 設定。

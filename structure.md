...
- `docs/` / `docs/en/`：繁中與英文 Markdown 文件來源；`contributing.md` 集中記錄主程式的本地驗證、測試增補及 viewport／button／card／list／dropdown／shadow／control 共用 UI 規範；`architecture.md` 說明前端 primitive 邊界；`deployment/cloudflare.md` 說明 Worker、workers.dev、API token 與 CORS 設定。
